...
#Define paths
RESULTS_PATH = '/Users/julianbatto-hokson/Desktop/Coding/Fantasy_Football_Model/SQL Code/'
OUTPUT_PATH = '/Users/julianbatto-hokson/Desktop/Coding/Fantasy_Football_Model/Python Code/Outputs/'
...
result_files = {
    'result_1': f'{RESULTS_PATH}Result1.csv',   #Draft Value Scoring Model
    'result_2': f'{RESULTS_PATH}Result2.csv',   #Fantasy Points by Position
    'result_3': f'{RESULTS_PATH}Result3.csv',   #Year-Over-Year Consistency
    'result_4': f'{RESULTS_PATH}Result4.csv',   #Top Performers (Multi-Year)
    'result_5': f'{RESULTS_PATH}Result5.csv',   #Top 5 QBs
    'result_6': f'{RESULTS_PATH}Result6.csv',   #Top 5 RBs
    'result_7': f'{RESULTS_PATH}Result7.csv',   #Top 5 WRs
    'result_8': f'{RESULTS_PATH}Result8.csv',   #Top 5 TEs
    'result_9': f'{RESULTS_PATH}Result9.csv',   #Position Predictability
    'result_10': f'{RESULTS_PATH}Result10.csv', #Player Consistency
    'result_11': f'{RESULTS_PATH}Result11.csv', #Draft Value Analysis
    'result_12': f'{RESULTS_PATH}Result12.csv', #Tier Classification
    'result_13': f'{RESULTS_PATH}Result13.csv', #Summary by Position
    'result_14': f'{RESULTS_PATH}Result14.csv', #Season Trends
    'result_15': f'{RESULTS_PATH}Result15.csv', #Complete Export Dataset
    'result_16': f'{RESULTS_PATH}Result16.csv'  #Draft Value Scoring Model
}
...
for i, bar in enumerate(bars):
    ...
    ax.text(width, bar.get_y() + bar.get_height()/2., f'{width:.1f}',
            ha='left', va='center', fontsize=11, fontweight='bold', bbox=dict(boxstyle='round', facecolor='wheat', alpha=0.5))

plt.tight_layout()
plt.savefig(f'{OUTPUT_PATH}01_fantasy_points_by_position.png', dpi=300, bbox_inches='tight')
...
plt.tight_layout()
plt.savefig(f'{OUTPUT_PATH}02_season_trends.png', dpi=300, bbox_inches='tight')
...
plt.tight_layout()
plt.savefig(f'{OUTPUT_PATH}03_top_performers.png', dpi=300, bbox_inches='tight')
...
qb_data = result_11[result_11['pos'] == 'QB'].sort_values('avg_pts', ascending=False)

x = np.arange(len(qb_data))
width = 0.35

bars1 = ax.bar(x - width/2, qb_data['avg_pts'], width, label='Average Points', color='steelblue', alpha=0.7, edgecolor='black')
bars2 = ax.bar(x + width/2, qb_data['std_dev'], width, label='Std Deviation', color='coral', alpha=0.7, edgecolor='black')
...
ax.set_xticks(x)
ax.set_xticklabels(qb_data['draft_round'])
ax.legend(fontsize=11)
ax.grid(True, alpha=0.3, axis='y')

plt.tight_layout()
plt.savefig(f'{OUTPUT_PATH}04_draft_value_by_round.png', dpi=300, bbox_inches='tight')
...
plt.tight_layout()
plt.savefig(f'{OUTPUT_PATH}05_tier_distribution.png', dpi=300, bbox_inches='tight')
...
for _, row in result_2_sorted.iterrows():
    print(f"   • {row['position']}: {row['avg_points']:.2f} avg pts (σ = {row['std_dev']:.2f})")
print()
...
for pos in ['QB', 'RB', 'WR', 'TE']:
    ...
    if len(pos_data) > 0:
        ...
        #Filter for players with avg pts above position average and high value_gained
...
for pos in ['QB', 'RB', 'WR', 'TE']:
    ...
    if len(pos_data) > 0:
        #Top performers: highest average pts (quality) + multiple seasons (consistency)
...
for pos in ['QB', 'RB', 'WR', 'TE']:
    ...
    if len(pos_data) > 0:
        ...
        #Higher avg_pts matters, having a low variance is key
...
for pos in ['QB', 'RB', 'WR', 'TE']:
    ...
    if len(pos_data) > 0:
        ...
        top_tiers = pos_data.head(15)
        
        #Add PPR-based tier classification based on points
        def classify_ppr_tier(pts):
            if pts >= PPR_ELITE:
                return 'Elite Performance (25+ pts)'
            elif pts >= PPR_STARTER:
                return 'Solid/Starter (12-20 pts)'
            elif pts >= PPR_FLEX:
                return 'Flex/Average (8-12 pts)'
            else:
                return 'Below Average (< 8 pts)'
...
for export_name, export_df in export_results.items():
    filename = f'{OUTPUT_PATH}{export_name}_analysis.csv'
